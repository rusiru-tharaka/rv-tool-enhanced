#!/usr/bin/env python3
"""
Offline AWS Pricing Data Downloader - FIXED VERSION
Downloads real AWS pricing data using correct AWS endpoints
Based on official AWS documentation - EBS pricing is within EC2 data
"""
...
class OfflineDataDownloader:
    """Optimized downloader for real AWS pricing data with correct endpoints"""
    
    # CORRECTED AWS Bulk Pricing Endpoints based on official documentation
    PRICING_ENDPOINTS = {
        ...
        # Note: EBS pricing is included in AmazonEC2 data, not separate
    ...
    # Essential services for RVTool migration analysis
    ESSENTIAL_SERVICES = ['index', 'AmazonEC2']  # EC2 includes EBS pricing
    ...
    def __init__(self, download_dir: str = None, chunk_size: int = 8192):
        ...
        logger.info("Note: EBS pricing is included in AmazonEC2 data")
    
    def verify_endpoint(self, service_code: str) -> bool:
        """Verify that an endpoint exists before downloading"""
        ...
        try:
            logger.info(f"Verifying endpoint for {service_code}...")
            response = self.session.head(url, timeout=30)
            
            if response.status_code == 200:
                logger.info(f"✅ Endpoint verified: {service_code}")
                return True
            elif response.status_code == 404:
                logger.error(f"❌ Endpoint not found (404): {service_code}")
                logger.error(f"   URL: {url}")
                return False
            else:
                logger.warning(f"⚠️  Unexpected status {response.status_code} for {service_code}")
                return False
                
        except requests.exceptions.RequestException as e:
            logger.error(f"❌ Endpoint verification failed for {service_code}: {e}")
            return False
    ...
    def download_service_data(self, service_code: str) -> bool:
        """Download data for a specific service with endpoint verification"""
        
        if service_code not in self.PRICING_ENDPOINTS:
            logger.error(f"Unknown service: {service_code}")
            return False
        
        # Verify endpoint exists before downloading
        if not self.verify_endpoint(service_code):
            return False
...
def download_essential_services(download_dir: str = None, services: List[str] = None) -> bool:
    """Download essential AWS pricing services with correct endpoints"""
    
    downloader = OfflineDataDownloader(download_dir)
    
    if services is None:
        services = downloader.ESSENTIAL_SERVICES  # Use corrected essential services
    
    logger.info("=" * 60)
    logger.info("OFFLINE AWS PRICING DATA DOWNLOAD - CORRECTED VERSION")
    logger.info("=" * 60)
    logger.info("Note: EBS pricing is included in AmazonEC2 data")
    logger.info(f"Services to download: {services}")
    ...
    if success_count == total_count:
        ...
        logger.info("2. EBS pricing will be extracted from EC2 data automatically")
        logger.info("3. Test local pricing service with real data")
        return True
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download AWS pricing data offline - CORRECTED VERSION")
    parser.add_argument("--dir", help="Download directory", default=None)
    parser.add_argument("--services", nargs="+", help="Services to download", 
                       default=None)  # Will use ESSENTIAL_SERVICES if None
    parser.add_argument("--all", action="store_true", help="Download all available services")
    parser.add_argument("--verify-only", action="store_true", help="Only verify endpoints, don't download")
    
    args = parser.parse_args()
    
    downloader = OfflineDataDownloader(args.dir)
    
    if args.verify_only:
        print("Verifying all AWS pricing endpoints...")
        for service in downloader.PRICING_ENDPOINTS.keys():
            downloader.verify_endpoint(service)
        sys.exit(0)
    
    if args.all:
        services = list(downloader.PRICING_ENDPOINTS.keys())
    elif args.services:
        services = args.services
    else:
        services = downloader.ESSENTIAL_SERVICES
    
    print(f"Starting offline download of services: {services}")
    print(f"Download directory: {args.dir or 'default (./offline_data)'}")
    print("\nNote: EBS pricing is included in AmazonEC2 data (AWS official structure)")
