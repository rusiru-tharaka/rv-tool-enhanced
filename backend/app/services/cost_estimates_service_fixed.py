"""
Cost Estimates Service - FIXED VERSION
Fixed to use Hybrid Pricing Service for Singapore region support
Maintains local pricing performance while adding AWS API fallback for missing regions
"""
...
from .hybrid_pricing_service import HybridPricingService
...
class CostEstimatesService:
    """
    FIXED Cost Estimates Service with Hybrid Pricing Support
    Supports Singapore region and all AWS regions through hybrid approach
    """
    
    def __init__(self):
        # Use hybrid pricing service instead of local-only
        self.pricing_service = HybridPricingService()
        self.recommendation_service = recommendation_service
        logger.info("Cost Estimates Service initialized with HYBRID pricing (Local + AWS API)")
    ...
    async def calculate_vm_cost_estimate(
        ...
        """
        FIXED: Calculate cost estimate with hybrid pricing support for all regions
        """
        logger.info(f"Calculating cost estimate for VM: {vm_spec.vm_name} in region: {tco_parameters.target_region}")
        ...
        try:
            ...
            # Get pricing data using hybrid service (supports all regions)
            ...
            if not instance_pricing:
                raise ValueError(f"No pricing data available for instance type {recommendation.instance_type} in region {tco_parameters.target_region}")
            
            # Get storage pricing using hybrid service
            ...
            if not storage_pricing:
                # Fallback to gp2 if gp3 not available
                storage_pricing = await self.pricing_service.get_storage_pricing(
                    "gp2", 
                    tco_parameters.target_region
                )
            ...
            if not storage_pricing:
                raise ValueError(f"No storage pricing data available for region {tco_parameters.target_region}")
            
            # Calculate compute costs with FIXED pricing models and OS detection
            compute_cost = await self._calculate_compute_cost_fixed(
                instance_pricing, 
                tco_parameters, 
            ...
            cost_estimate = VMCostEstimate(
                ...
                pricing_plan=await self._get_pricing_plan_name_fixed(instance_pricing, tco_parameters, vm_spec.workload_type),
            ...
            logger.info(f"Cost estimate calculated for {vm_spec.vm_name}: ${total_monthly:.2f}/month (Instance: {recommendation.instance_type}, Region: {tco_parameters.target_region})")
    ...
    async def _calculate_compute_cost_fixed(
        self, 
        ...
        """FIXED compute cost calculation with proper Savings Plans support"""
        ...
        # Get hourly rate based on pricing model with FIXED Savings Plans support
        pricing_plan, hourly_rate = await self._get_hourly_rate_for_model_fixed(
            instance_pricing, 
            pricing_model, 
            tco_parameters, 
            workload_type
        ...
        logger.info(f"FIXED compute cost: {pricing_plan} @ ${hourly_rate:.4f}/hr * {effective_hours:.1f}hrs = ${os_adjusted_cost:.2f} (OS: {os_type})")
        
        return os_adjusted_cost
    
    async def _get_hourly_rate_for_model_fixed(
        self, 
        instance_pricing: InstancePricing, 
        pricing_model: str, 
        tco_parameters: TCOParameters, 
        ...
        """FIXED hourly rate calculation with proper Savings Plans support"""
        ...
        if pricing_model == "on_demand":
            ...
            # FIXED: Use hybrid pricing service for Savings Plans
            try:
                ...
                if savings_plans_pricing and len(savings_plans_pricing) > 0:
                    ...
                    if hasattr(instance_pricing, 'reserved_1yr_no_upfront') and instance_pricing.reserved_1yr_no_upfront:
                        ...
                        return "On-Demand (Savings Plans Fallback)", instance_pricing.on_demand_hourly
                ...
                if hasattr(instance_pricing, 'reserved_1yr_no_upfront') and instance_pricing.reserved_1yr_no_upfront:
                    ...
                    return "On-Demand (Savings Plans Error)", instance_pricing.on_demand_hourly
        
        # Default fallback
        return "On-Demand (Default)", instance_pricing.on_demand_hourly
    
    async def _get_pricing_plan_name_fixed(
        self, 
        instance_pricing: InstancePricing, 
        tco_parameters: TCOParameters, 
        workload_type: WorkloadType
    ...
    def _generate_optimization_notes(
        ...
        if tco_parameters.target_region not in {'us-east-1', 'us-west-2', 'eu-west-1'}:
    ...
    def _create_fallback_cost_estimate(self, vm_spec: VMSpecification, error_message: str) -> VMCostEstimate:
        ...
        return VMCostEstimate(
            ...
            recommended_instance_size="unknown",
            recommended_instance_type="unknown",
            pricing_plan="Error",
            workload_type=vm_spec.workload_type.value,
            ...
            cost_optimization_notes=f"Cost calculation failed: {error_message}"
        )

# Create singleton instance
cost_estimates_service_fixed = CostEstimatesService()
