"""
Cost Estimates Service - Hardcoded Parameters Version
...
# Official RVTools Column Mapping
RVTOOLS_COLUMNS = {
    'vm_name': 'VM',
    'cpu_cores': 'CPUs', 
    'memory_mib': 'Memory',
...
class CostEstimatesServiceHardcoded:
    """
    Enhanced Cost Estimates Service with hardcoded parameters
    Forces 3-Year RI for Production and On-Demand 50% for Non-Production
    """
    ...
    def __init__(self):
        ...
        logger.info("CostEstimatesServiceHardcoded initialized with enhanced pricing service")
    ...
    async def analyze_cost_estimates(
        ...
        """
        logger.info(f"🔧 HARDCODED PARAMETERS: Starting cost analysis for session {session_id}")
    ...
    async def _calculate_detailed_estimates_parallel(
        ...
        for vm_spec in vm_specs:
            try:
                recommendation = rec_lookup.get(vm_spec.vm_name)
...
cost_estimates_service_hardcoded = CostEstimatesServiceHardcoded()
