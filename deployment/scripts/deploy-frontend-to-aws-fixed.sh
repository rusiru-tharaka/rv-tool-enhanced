...
# RVTool Enhanced Frontend Deployment Script - FIXED VERSION
# Deploys the React/Vite frontend to AWS S3 + CloudFront with dependency fixes
# Version: 1.1 - Production Deployment with Fixes
...
EOF
    ...
    # Update vite.config.ts for production (without terser to avoid issues)
...
EOF
    print_status "Vite configuration updated (using esbuild instead of terser)"
...
main() {
    print_header "🚀 RVTool Enhanced Frontend Deployment - FIXED"
    echo ""
    print_info "Starting frontend deployment process with dependency fixes..."
    echo ""
