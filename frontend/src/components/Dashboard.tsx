...
const Dashboard: React.FC = () => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      // Set retry flag if we're retrying
      if (isRetrying) {
        setIsRetrying(false);
      }
      ...
      if (error.message === 'Network Error' || error.message?.includes('Connection refused')) {
        setUploadError('Cannot connect to the AWS backend server. Please check your network connection and try again.');
      } else if (error.message?.includes('timeout')) {
        setUploadError('The connection to the AWS backend timed out. Please try again or use a smaller file.');
      } else {
              ...
              {uploadError && (
                <div className="mt-4 p-3 bg-red-50 rounded-md">
                  ...
                  {uploadedFile && (
                    <button 
                      onClick={handleRetry}
                      className="mt-2 flex items-center text-sm text-primary-600 hover:text-primary-500"
                    >
